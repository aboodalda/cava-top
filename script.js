...
document.getElementById('cartSendBtn').addEventListener('click', async () => {
  ...
  const custTable = document.getElementById('custTable').value.trim();

  if (!custName) { document.getElementById('custName').focus(); return showToastMini('اكتب اسمك'); }
  if (!custTable) { document.getElementById('custTable').focus(); return showToastMini('اكتب رقم الطاولة'); }
  ...
  ids.forEach(id => {
    ...
    total += sub;
    orderItems.push({ name: it.name, price: it.price, qty, subtotal: sub, categoryId: it.categoryId });
  });

  // save to Firestore so it shows up live in the admin dashboard
  try {
    await db.collection('orders').add({
      customerName: custName,
      tableNumber: custTable,
      items: orderItems,
      total,
      status: 'new',
  ...
  const CATEGORY_EMOJI = {
    hot: '☕', cold: '🥤', mains: '🍽️', sweets: '🍰',
    argela: '💨', ieckreem: '🍨', fish: '🐟', saltat: '🥗'
  };
  const orderNumber = String(Date.now()).slice(-4);
  const timeStr = new Date().toLocaleTimeString('ar-EG', { hour: '2-digit', minute: '2-digit' });
  const divider = '────────────────';

  let msg = `🔥 طلب جديد | Cava Top #${orderNumber}\n${divider}\n`;
  msg += `👤 ${custName}\n`;
  msg += `🪑 طاولة رقم ${custTable}\n\n`;
  orderItems.forEach(oi => {
    const emoji = CATEGORY_EMOJI[oi.categoryId] || '🍴';
    msg += `${emoji} ${oi.name} × ${oi.qty} = ${oi.subtotal.toFixed(2)} ₪\n`;
  });
  msg += `${divider}\n`;
  msg += `💰 المجموع: ${total.toFixed(2)} ₪\n\n`;
  msg += `⏱️ ${timeStr}\n`;
  msg += `شكراً لطلبك 🙏`;
