...
let currentImageData = ''; // base64 data URL for the item being added/edited
...
function drawAndEncode(file, maxDim, quality) {
...
async function compressImageFile(file, maxBytes = MAX_IMAGE_BYTES) {
  let maxDim = 1000;
  let dataUrl = null;
  for (let attempt = 0; attempt < 6; attempt++) {
    let quality = 0.85;
    dataUrl = await drawAndEncode(file, maxDim, quality);
    while (dataUrlSizeBytes(dataUrl) > maxBytes && quality > 0.25) {
      quality -= 0.15;
      dataUrl = await drawAndEncode(file, maxDim, quality);
    }
  ...
  return dataUrl; // best effort even if slightly over
}

function showImagePreview(dataUrl) {
  const row = document.getElementById('imagePreviewRow');
  const img = document.getElementById('itImagePreview');
  const sizeLabel = document.getElementById('imagePreviewSize');
  if (!dataUrl) {
  ...
  img.src = dataUrl;
  const kb = Math.round(dataUrlSizeBytes(dataUrl) / 1024);
  sizeLabel.textContent = `حجم الصورة تقريباً: ${kb} كيلوبايت`;
  row.style.display = 'flex';
}

document.getElementById('itImageFile').addEventListener('change', async e => {
  ...
  try {
    ...
    showToast('تم ضغط الصورة بنجاح');
  } catch (err) {
    console.error(err);
    showToast('تعذر معالجة الصورة، جرب صورة ثانية');
  }
  e.target.value = '';
});

document.getElementById('itImageRemoveBtn').addEventListener('click', () => {
  currentImageData = '';
  showImagePreview('');
});
