...
async function compressImageFile(file, maxDim = 1200, quality = 0.82) {
  const dataUrl = await drawAndEncode(file, maxDim, quality);
...
async function uploadToCloudinary(blob) {
  ...
  // insert an auto-optimization transform (format + quality + width cap) into the delivery URL
  return data.secure_url.replace('/upload/', '/upload/w_700,q_auto,f_auto/');
}
...
function showImagePreview(url) {
  ...
  img.src = url;
  sizeLabel.textContent = 'مرفوعة على Cloudinary ✓';
...
document.getElementById('itImageFile').addEventListener('change', async e => {
  ...
  try {
    ...
    console.error(err);
    showToast('تعذر رفع الصورة — تأكد من إعدادات Cloudinary');
  }
  e.target.value = '';
});
